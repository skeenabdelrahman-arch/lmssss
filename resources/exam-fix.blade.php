    ...
    <script>
        ...
        function showLoading(message = 'جاري المعالجة...') {
            document.getElementById('result').innerHTML = 
                '<div class="status loading"><span class="spinner"></span> ' + message + '</div>';
        }

        function showSuccess(message, data = null) {
            let html = '<div class="status success">' + message + '</div>';
            if (data) {
                html += '<div style="background: #f0f0f0; padding: 15px; margin-top: 10px; border-radius: 4px; overflow-x: auto; font-size: 12px;">';
        ...
        function fixDatabase() {
            ...
            fetch('{{ route("exam.fix.constraints") }}', {
                method: 'POST',
                headers: {
        ...
        function checkConstraints() {
            ...
            .then(data => {
                if (data.success && data.data && data.data.length > 0) {
                    ...
                    showSuccess('✅ تم الحصول على معلومات القيود بنجاح! (' + data.data.length + ' قيد)');
                } else {
                    showError(data.message || 'لم يتم العثور على قيود');
                }
            })
