...
<head>
    ...
    <meta property="og:image" content="{{ asset(logo_path()) }}" />
    <link rel="shortcut icon" href="{{ asset(logo_path()) }}" type="image/x-icon">
    
    
    <title>@yield('title', site_name())</title>
    
    <!-- Dynamic Fonts -->
    @if(heading_font() == 'Cairo')
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;600;700;900&display=swap" rel="stylesheet">
    @elseif(heading_font() == 'Almarai')
    <link href="https://fonts.googleapis.com/css2?family=Almarai:wght@300;400;700;800&display=swap" rel="stylesheet">
    @elseif(heading_font() == 'Amiri')
    ...
    @if(body_font() != heading_font())
        @if(body_font() == 'Cairo')
        <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;600;700;900&display=swap" rel="stylesheet">
        @elseif(body_font() == 'Almarai')
        <link href="https://fonts.googleapis.com/css2?family=Almarai:wght@300;400;700;800&display=swap" rel="stylesheet">
        @elseif(body_font() == 'Amiri')
        <link href="https://fonts.googleapis.com/css2?family=Amiri:wght@400;700&display=swap" rel="stylesheet">
        @endif
    @endif
    ...
    <meta name="description" content="@yield('description', setting('site_description', config('seo.site_description', 'منصة تعليمية متخصصة')))" />
    <meta name="keywords" content="@yield('keywords', setting('site_keywords', config('seo.site_keywords', 'تعليم, أونلاين')))" />
    
    {{-- Open Graph Meta Tags --}}
    <meta property="og:title" content="@yield('title', config('seo.site_title'))" />
    ...
    <meta name="twitter:description" content="@yield('description', setting('site_description', config('seo.site_description', '')))" />
    ...
    <script type="application/ld+json">
    {
        ...
        "name": "{{ site_name() }}",
        "description": "{{ setting('site_description', config('seo.site_description', '')) }}",
        "url": "{{ url('/') }}",
    ...
    {{-- Google Analytics --}}
    @if(config('seo.google_analytics'))
    <!-- Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id={{ config('seo.google_analytics') }}"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());
        gtag('config', '{{ config('seo.google_analytics') }}');
    </script>
    @endif
    
    {{-- Facebook Pixel --}}
    @if(config('seo.facebook_pixel'))
    <!-- Facebook Pixel Code -->
    <script>
        ...
        fbq('init', '{{ config('seo.facebook_pixel') }}');
        fbq('track', 'PageView');
    </script>
    <noscript><img height="1" width="1" style="display:none"
        src="https://www.facebook.com/tr?id={{ config('seo.facebook_pixel') }}&ev=PageView&noscript=1"
    /></noscript>
    @endif
    ...
    <link rel="stylesheet" href="{{ URL::asset('css/admin-enhancements.css') }}">
    <!-- Modern Sidebar CSS -->
    <link rel="stylesheet" href="{{ URL::asset('css/modern-sidebar.css') }}">
    
    <!-- Lazy Loading JS -->
    <script src="{{ URL::asset('js/lazy-loading.js') }}" defer></script>
...
<body>
    ...
    <aside class="modern-sidebar" id="modernSidebar">
        ...
        <div class="sidebar-logo" style="position: relative;">
            <a href="{{url('/')}}" aria-label="الرئيسية">
                <img src="{{ asset(logo_path()) }}" alt="Logo">
            </a>
            <!-- Desktop Toggle -->
        ...
        <div class="sidebar-profile">
            <a href="{{route('student_profile', ['student_id' => Auth::guard('student')->user()->id])}}">
                @if (Auth::guard('student')->user()->image)
                    <img src="{{url('upload_files/'.Auth::guard('student')->user()->image)}}" alt="Profile" class="sidebar-profile-img">
                @else
                    <img src="{{url('front/assets/images/avatar.jpg')}}" alt="Profile" class="sidebar-profile-img">
                @endif
            </a>
            <h6 class="sidebar-profile-name">{{ Auth::guard('student')->user()->first_name }} {{ Auth::guard('student')->user()->second_name }}</h6>
        ...
        <nav class="sidebar-nav">
            ...
            @if (Auth::guard('student')->check())
            <div class="sidebar-nav-item">
                <a href="{{route('student.dashboard')}}" class="sidebar-nav-link {{ request()->is('dashboard') ? 'active' : '' }}">
            ...
            <div class="sidebar-nav-item">
                <a href="{{ route('about.teacher') }}" class="sidebar-nav-link {{ request()->is('about-teacher') ? 'active' : '' }}">
            ...
            <div class="sidebar-nav-item">
                <a href="{{route('faq.index')}}" class="sidebar-nav-link {{ request()->is('faq*') ? 'active' : '' }}">
            ...
            @if (Auth::guard('student')->check())
            <div class="sidebar-nav-item">
                <a href="{{route('courses.index')}}" class="sidebar-nav-link {{ request()->is('courses*') ? 'active' : '' }}">
            ...
            <div class="sidebar-nav-item">
                <a href="{{route('bundles.index')}}" class="sidebar-nav-link {{ request()->is('bundles*') || request()->is('bundle*') ? 'active' : '' }}">
            ...
            <div class="sidebar-nav-item">
                <a href="{{route('publicExam.index')}}" class="sidebar-nav-link {{ request()->is('public-exam*') ? 'active' : '' }}">
            ...
            <div class="sidebar-dropdown">
                <button class="sidebar-dropdown-toggle {{ request()->is('activate-code*') || request()->is('activate-instructions*') ? 'active' : '' }}" onclick="toggleDropdown('activationDropdown')">
                ...
                <div class="sidebar-dropdown-content" id="activationDropdown">
                    <a href="{{route('activation_code.index')}}" class="sidebar-dropdown-link">
                        <i class="fas fa-key"></i>
                        <span>تفعيل كود</span>
                    </a>
                    <a href="{{route('student.activate.instructions')}}" class="sidebar-dropdown-link">
            ...
            <div class="sidebar-nav-item">
                <a href="{{route('payment.history')}}" class="sidebar-nav-link {{ request()->is('payment/history*') ? 'active' : '' }}">
            ...
            <div class="sidebar-nav-item">
                <a href="{{route('notifications.index')}}" class="sidebar-nav-link {{ request()->is('notifications*') ? 'active' : '' }}">
        ...
        <div class="sidebar-footer">
            ...
            @if (Auth::guard('student')->check())
            <a href="{{route('studentLogout')}}" class="sidebar-footer-btn secondary">
                <i class="fas fa-sign-out-alt"></i>
                <span>تسجيل خروج</span>
            </a>
            @else
            <a href="{{route('studentLogin')}}" class="sidebar-footer-btn primary">
                <i class="fas fa-sign-in-alt"></i>
                <span>تسجيل الدخول</span>
            </a>
            <a href="{{route('studentSignup')}}" class="sidebar-footer-btn secondary">
    ...
    <div class="content-wrapper">
        ...
        <div class="mobile-top-toggle" style="display: flex; align-items: center; justify-content: space-between; gap: 10px;">
            ...
            <div class="mini-header-actions" style="display: flex; align-items: center; gap: 8px;">
                @if (Auth::guard('student')->check())
                    <a href="{{ url('/') }}" class="btn btn-sm" style="background: rgba(116, 36, 169, 0.08); color: var(--primary-color); border: 1px solid rgba(116,36,169,0.2); border-radius: 10px; padding: 8px 12px; font-weight: 600; text-decoration: none;">
                        <i class="fas fa-home"></i> الرئيسية
                    </a>
                    <a href="{{route('student_profile', ['student_id' => Auth::guard('student')->user()->id])}}" class="btn btn-sm" style="background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: #fff; border: none; border-radius: 10px; padding: 8px 12px; font-weight: 600; text-decoration: none; display: flex; align-items: center; gap: 6px;">
                        @if (Auth::guard('student')->user()->image)
                            <img src="{{url('upload_files/'.Auth::guard('student')->user()->image)}}" alt="" style="width: 22px; height: 22px; border-radius: 50%; object-fit: cover; border: 1px solid rgba(255,255,255,0.6);">
                        @else
                ...
                @else
                    <a href="{{route('studentLogin')}}" class="btn btn-sm" style="background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: #fff; border: none; border-radius: 10px; padding: 8px 12px; font-weight: 600; text-decoration: none;">
                        <i class="fas fa-sign-in-alt"></i> تسجيل الدخول
                    </a>
                    <a href="{{route('studentSignup')}}" class="btn btn-sm" style="background: #fff; color: var(--primary-color); border: 2px solid var(--primary-color); border-radius: 10px; padding: 8px 12px; font-weight: 600; text-decoration: none;">
                        ...
                        <div class="modal-body" style="padding: 30px;">
                            <div class="alert alert-danger" style="border-right: 5px solid #dc3545;">
                                ...
                                <ul class="mb-0" style="font-size: 16px; line-height: 2;">
                                    @foreach($deactivatedSubscriptions as $sub)
                                        <li>
                                            <strong>{{ $sub->month ? $sub->month->name : 'كورس غير معروف' }}</strong>
                                            @if($sub->deactivation_reason)
                                                <br>
                                                <small class="text-muted">السبب: {{ $sub->deactivation_reason }}</small>
                                            @endif
                                        </li>
                                    @endforeach
                                </ul>
                            ...
                            <div class="alert alert-info mt-3">
                                ...
                                <p class="mb-0 mt-2">
                                    ...
                                    @if(whatsapp_number())
                                        <br>
                                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', whatsapp_number()) }}" target="_blank" class="btn btn-success btn-sm mt-2">
                                            <i class="fab fa-whatsapp me-1"></i> واتساب: {{ whatsapp_number() }}
                                        </a>
                                    @endif
                                </p>
                            </div>
    ...
    <footer class="main-footer">
        <div class="footer-content">
            <div class="row">
                <div class="col-lg-4 col-md-6 footer-section">
                    ...
                    <p style="color: rgba(255,255,255,0.9); line-height: 1.8;">
                        {{ footer_text() ?: ('منصة تعليمية متخصصة في مادة ' . subject_name() . ' للثانوية العامة، نسعى لتقديم أفضل تجربة تعليمية لطلابنا.') }}
                    </p>
                    <div class="social-icons">
                        @if(social_url('facebook'))
                        <a href="{{ social_url('facebook') }}" target="_blank"><i class="fab fa-facebook-f"></i></a>
                        @endif
                        @if(social_url('twitter'))
                        <a href="{{ social_url('twitter') }}" target="_blank"><i class="fab fa-twitter"></i></a>
                        @endif
                        @if(social_url('instagram'))
                        <a href="{{ social_url('instagram') }}" target="_blank"><i class="fab fa-instagram"></i></a>
                        @endif
                        @if(whatsapp_number())
                        <a href="https://wa.me/{{ str_replace(['+', ' ', '-'], '', whatsapp_number()) }}" target="_blank"><i class="fab fa-whatsapp"></i></a>
                        @endif
                        @php
                        ...
                        @endif
                        @if(telegram_url())
                        <a href="{{ telegram_url() }}" target="_blank"><i class="fab fa-telegram"></i></a>
                        @endif
                        @if(social_url('linkedin'))
                        <a href="{{ social_url('linkedin') }}" target="_blank"><i class="fab fa-linkedin"></i></a>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-lg-2 col-md-6 footer-section">
                    ...
                    <ul class="footer-links">
                        ...
                        <li><a href="{{route('courses.index')}}"><i class="fas fa-book"></i> الكورسات</a></li>
                        <li><a href="{{route('publicExam.index')}}"><i class="fas fa-file-alt"></i> الامتحانات المجانية</a></li>
                        @if(privacy_policy())
                        <li><a href="{{route('privacy.policy')}}"><i class="fas fa-shield-alt"></i> سياسة الخصوصية</a></li>
                        @endif
                        @if(terms_of_service())
                        <li><a href="{{route('terms.of.service')}}"><i class="fas fa-gavel"></i> شروط الاستخدام</a></li>
                        @endif
                    </ul>
                </div>
                ...
                <div class="col-lg-3 col-md-6 footer-section">
                    ...
                    <ul class="footer-links">
                        ...
                        <li><a href="{{route('studentLogin')}}"><i class="fas fa-user-graduate"></i> {{ $grade['name'] }}</a></li>
                        @endforeach
                        ...
                        @if(!empty($extraLinks))
                            ...
                            @foreach($extraLinks as $label => $url)
                                @if(!empty($url))
                                <li>
                ...
                <div class="col-lg-3 col-md-6 footer-section">
                    ...
                    <ul class="footer-links">
                        @if(whatsapp_number())
                        <li><a href="https://wa.me/{{ str_replace(['+', ' ', '-'], '', whatsapp_number()) }}" target="_blank"><i class="fab fa-whatsapp"></i> واتساب: {{ whatsapp_number() }}</a></li>
                        @endif
                        @if(phone_number())
                        <li><a href="tel:{{ phone_number() }}"><i class="fas fa-phone"></i> {{ phone_number() }}</a></li>
                        @endif
                        @if(contact_email())
                        <li><a href="mailto:{{ contact_email() }}"><i class="fas fa-envelope"></i> {{ contact_email() }}</a></li>
                        @endif
                    </ul>
                </div>
            </div>
            ...
            <div class="footer-bottom">
                <p>&copy; {{ date('Y') }} - {{ teacher_full_name() ?: teacher_name() }}. {{ footer_copyright() }}</p>
                @if(footer_text())
                <p style="margin-top: 10px; font-size: 14px;">{{ footer_text() }}</p>
                @endif
                <div class="mt-3" style="font-size: 12px; opacity: 0.8;">
                    @if(privacy_policy())
                    <a href="{{route('privacy.policy')}}" style="color: rgba(255,255,255,0.8); text-decoration: none; margin-left: 15px;">سياسة الخصوصية</a>
                    @endif
                    @if(terms_of_service())
                    <a href="{{route('terms.of.service')}}" style="color: rgba(255,255,255,0.8); text-decoration: none; margin-left: 15px;">شروط الاستخدام</a>
                    @endif
                </div>
                ...
                @endphp
                @if(!empty($developerFacebook) || !empty($developerWhatsapp))
                <div class="mt-3" style="font-size: 12px; opacity: 0.7; text-align: center;">
                    <span>{{ $developerText }}</span>
                    @if(!empty($developerFacebook))
                        <a href="{{ $developerFacebook }}" target="_blank" style="color: rgba(255,255,255,0.8); text-decoration: none; margin: 0 8px;">
                            <i class="fab fa-facebook"></i>
                        </a>
                    @endif
                    @if(!empty($developerWhatsapp))
                        <a href="https://wa.me/{{ str_replace(['+', ' ', '-'], '', $developerWhatsapp) }}" target="_blank" style="color: rgba(255,255,255,0.8); text-decoration: none; margin: 0 8px;">
    ...
    @if(whatsapp_number())
    <a href="https://wa.me/{{ str_replace(['+', ' ', '-'], '', whatsapp_number()) }}" target="_blank" class="contact-float-btn" title="تواصل معنا">
        <i class="fab fa-whatsapp"></i>
    </a>
    @endif
    ...
    <script src="{{ URL::asset('js/admin-enhancements.js') }}"></script>
