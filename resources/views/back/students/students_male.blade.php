...
<div class="page-header-modern">
    <h4><i class="fas fa-mars me-2"></i> الطلاب الذكور</h4>
</div>
@endsection
@section('content')
            ...
            <table id="datatable" class="table table-hover" data-page-length="25">
                ...
                <tbody>
                    @foreach ($maleStudents as $maleStudent)
                        <tr>
                            <td>
                                <a href="{{url('student-profile/'.$maleStudent->id)}}" class="student-name-link">
                                    {{ $maleStudent->first_name }}
                                    {{ $maleStudent->second_name }}
                                    {{ $maleStudent->third_name }}
                                    {{ $maleStudent->forth_name }}
                                </a>
                            </td>
                            <td><span class="badge-modern badge-modern-primary">{{ $maleStudent->grade }}</span></td>
                            <td>{{ $maleStudent->student_phone }}</td>
                            <td>
                                @if($maleStudent->image)
                                    <img src="{{ url('upload_files/' . $maleStudent->image)}}" class="student-image" alt="صورة الطالب">
                                @else
                                    <span class="badge-modern badge-modern-danger">لا توجد صورة</span>
                                @endif
                            </td>
                            <td><span class="password-code">{{ $maleStudent->password }}</span></td>
                            <td>
                                <div class="action-buttons">
                                    <a href="{{ url('students-male/edit/'.$maleStudent->id) }}" class="btn btn-modern btn-modern-warning btn-sm" title="تعديل">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a onclick="return confirm('هل انت متاكد من الحذف ؟ ')" href="{{url('Student/delete/'.$maleStudent->id)}}" class="btn btn-modern btn-modern-danger btn-sm" title="حذف">
